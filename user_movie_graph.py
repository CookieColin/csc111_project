...
def load_user_movie_data(file_path: str = "user_movie_ratings.csv") -> list[dict[str, str | float]]:
...
def visualize_graph_plotly(graph: nx.Graph, target_user: str | None = None, output_file: str = '') -> None:
    """Visualize the user-movie graph using plotly."""
    pos = nx.spring_layout(graph, seed=42)
    x_nodes, y_nodes, labels, colors = [], [], [], []
...
if __name__ == "__main__":
    user_movie_data = load_user_movie_data("user_movie_ratings.csv")
    user_graph = build_user_movie_graph(user_movie_data)

    print("✅ Graph built!")
    print("📊 Number of nodes:", user_graph.number_of_nodes())
    print("🔗 Number of edges:", user_graph.number_of_edges())

    target_user_id = input("🔍 Enter a user ID (e.g., 1): ").strip()
    similar_users = find_similar_users(user_graph, target_user_id)

    if not similar_users:
        print(f"⚠️ User '{target_user_id}' not found in the graph or has no similar users.")
    else:
        print(f"\\n👥 Top similar users to {target_user_id}:")
        for user, score in similar_users:
            print(f"  {user} (similarity: {score:.2f})")

    recommended_movies = recommend_movies(user_graph, target_user_id)
    print(f"\\n🎬 Recommended movies for {target_user_id}:")
    for movie, score in recommended_movies:
        print(f"  {movie} (score: {score:.2f})")

    visualize_graph_plotly(user_graph, target_user=target_user_id)